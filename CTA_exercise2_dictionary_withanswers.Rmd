---
title: "CTA-ED Exercise 2: dictionary-based methods (with answers)"
...
dfm_tabloid_nrc %>%
  kable(booktabs = TRUE) %>% # the kable() command (from the kableextra package) turns the dataset into a nice-looking html table
  kable_styling(c("striped", "condensed")) # this is just to make the table prettier and easier to read. You can skip this line if you want to keep it simple.
```
...
## Extra code
Because some of you may be interested in analysing sentiment not just by newspaper but by newspaper _and_ date, I include code to do this.
```{r}
...
The Mirror is clearly way more negative overall, but also more dispersed whereas the Times, the Telegraph, the Guardian and Metro all show compact and stable sentiment over time. 
